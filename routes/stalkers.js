...
// Получить всех сталкеров
router.get('/', authenticateToken, async (req, res) => {
  try {
    const { searchBy, searchTerm } = req.query;
    let query = 'SELECT * FROM stalkers';
    const params = [];
...
router.post('/', authenticateToken, requireAdmin, upload.single('photo'), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO stalkers (callsign, full_name, face_id, note, photo_path) VALUES ($1, $2, $3, $4, $5) RETURNING *',
      [callsign, fullName, faceId, note, photoPath]
    );
...
router.put('/:id', authenticateToken, requireAdmin, upload.single('photo'), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE stalkers SET callsign = $1, full_name = $2, face_id = $3, note = $4, photo_path = $5, updated_at = CURRENT_TIMESTAMP WHERE id = $6 RETURNING *',
      [callsign, fullName, faceId, note, photoPath, id]
    );
...
router.delete('/:id', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    // Проверяем, существует ли сталкер
    const existingStalker = await pool.query(
      'SELECT photo_path FROM stalkers WHERE id = $1',
      [id]
    );
    ...
    await pool.query('DELETE FROM stalkers WHERE id = $1', [id]);
